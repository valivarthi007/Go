...
resource "aws_security_group" "eks_cluster_sg" {
  count        = length(aws_security_group.eks_cluster_sg) == 0 ? 1 : 0
  name         = "eks-cluster-sg"
...
resource "aws_eks_cluster" "my_cluster" {
  count       = length(aws_eks_cluster.my_cluster) == 0 ? 1 : 0
  name        = "my-cluster"
...
resource "aws_eks_node_group" "my_node_group" {
  count             = length(aws_eks_node_group.my_node_group) == 0 ? 1 : 0
  cluster_name      = aws_eks_cluster.my_cluster[0].name
